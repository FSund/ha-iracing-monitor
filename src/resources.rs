...
pub fn load_as_rgba(image_bytes: &[u8]) -> Result<image::RgbaImage> {
    ...
    let icon_with_format = icon.with_guessed_format().context("Failed to guess image format")?;
    let pixels = icon_with_format.decode().context("Failed to decode image")?.to_rgba8();
    Ok(pixels)
}